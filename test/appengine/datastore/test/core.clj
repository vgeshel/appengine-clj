(ns appengine.datastore.test.core
  (:use clojure.test appengine.datastore.core appengine.datastore.keys appengine.test)
...
(datastore-test test-entity->map-with-key
  (let [continent (entity->map (doto (Entity. (create-key "continent" "eu")) (.setProperty "name" "Europe")))]
    (let [key (:key continent)]
      (is (.isComplete key))
      (is (nil? (.getParent key)))
      (is (= (.getKind key) "continent"))
...
(datastore-test test-entity->map-with-parent
  (let [continent (entity->map (doto (Entity. (create-key "continent" "eu")) (.setProperty "name" "Europe")))
        country (entity->map (doto (Entity. (create-key (:key continent) "country" "es")) (.setProperty "name" "Spain")))]    
    (let [key (:key country)]
      (is (.isComplete key))
...
(datastore-test test-entity->map-with-exisiting-entity
  (let [continent (entity->map (map->entity (create-entity {:key (create-key "continent" "eu") :name "Europe"})))]
    (let [key (:key continent)]
      (is (.isComplete key))
      (is (nil? (.getParent key)))
      (is (= (.getKind key) "continent"))
...
(datastore-test test-entity->map-with-exisiting-entity-and-parent
  (let [continent (create-entity {:key (create-key "continent" "eu") :name "Europe"})
        country (entity->map (map->entity (create-entity {:key (create-key (:key continent) "country" "es") :name "Spain"})))]
    (let [key (:key country)]
      (is (.isComplete key))
      (is (= (.getParent key) (:key continent)))
      (is (= (.getKind key) "country"))
...
(datastore-test test-key->string  
  (is (= (key->string (create-key "person" 1)) "agR0ZXN0cgwLEgZwZXJzb24YAQw"))
...
(datastore-test test-map->entity-with-key
  (let [entity (map->entity {:key (create-key "continent" "eu") :name "Europe"})]
    (is (= (class entity) Entity))
    (is (= (.getKind entity) "continent"))
    (is (= (.. entity getKey getKind) "continent"))
    (is (= (.. entity getKey getId) 0))
...
(datastore-test test-map->entity-with-parent-and-key
  (let [continent (create-entity {:key (create-key "continent" "eu") :name "Europe"})
        country (map->entity (create-entity {:key (create-key (:key continent) "country" "es") :name "Spain"}))]
    (let [key (.getKey country)]
      (is (.isComplete key))
      (is (= (.getParent key) (:key continent)))
      (is (= (.getKind key) "country"))
...
(datastore-test test-create-entity-with-string-key
  (let [country (create-entity {:key (create-key "country" "de") :name "Germany"})]
...
(datastore-test test-put-entity
  (let [continent (put-entity {:name "Europe" :key (create-key "continent" "eu")})]
    ...
    (is (= (count (find-all (Query. "continent"))) 1))
    (let [country (put-entity {:name "Germany" :key (create-key (:key continent) "country" "de")})]
...
(datastore-test test-update-entity
  (let [country (put-entity {:key (create-key "country" "de") :name "Deutschland"})]
...
(datastore-test test-update-entity-with-parent
  (let [continent (create-entity {:key (create-key "continent" "eu") :name "Europe"})
        country (create-entity {:key (create-key (:key continent) "country" "de") :name "Deutschland"})]
    (let [country (update-entity country {:name "Germany"})]
      (is (= (:name country) "Germany"))
      (is (= (count (find-all (Query. "country"))) 1)))))

(datastore-test test-properties
  (let [record {:key (create-key "person" 1) :name "Bob"}]
